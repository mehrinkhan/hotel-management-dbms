...
elseif ($action == 'get_initial_data') {
    ...
    while ($res && $r = $res->fetch_assoc()) {
        ...
        // BIDIRECTIONAL AUTO-FIX: Ensure availability status stays in sync with current bookings
        if ($status === 'Occupied' && !$occData) {
            $conn->query("UPDATE room SET Availability_Status = 'Available' WHERE Room_ID = $rid");
            $status = 'Available';
        } elseif ($status === 'Available' && $occData) {
            $conn->query("UPDATE room SET Availability_Status = 'Occupied' WHERE Room_ID = $rid");
            $status = 'Occupied';
        }
    ...
    // 4. Guest Database (RECORDS): Simplified Resident List (No duplicates per booking)
    if ($role === 'admin') {
        $dbSql = "SELECT Customer_ID, Name as guestName, Email as email, Phone as phone FROM customer ORDER BY Customer_ID DESC";
    } else {
        $dbSql = "SELECT Customer_ID, Name as guestName, Email as email, Phone as phone FROM customer WHERE Customer_ID = $userId";
    }
    ...
    $res = $conn->query("SELECT b.Booking_ID, c.Customer_ID, c.Name as guestName, r.Room_Type as suite, 
                         ...
                         WHERE b.Booking_Status IN ('Confirmed', 'Completed') AND i.Total_Amount > 0
    ...
    // 8. Pending Bookings for Admin
    $pending = [];
    if ($role === 'admin') {
        $res = $conn->query("SELECT b.Booking_ID, c.Name as guestName, r.Room_Type as suite, 
                                    b.Check_In_Date as date, DATEDIFF(b.Check_Out_Date, b.Check_In_Date) as nights,
                                    i.Total_Amount 
                             FROM booking b
                             JOIN customer c ON b.Customer_ID = c.Customer_ID
                             JOIN room r ON b.Room_ID = r.Room_ID
                             JOIN invoice i ON b.Booking_ID = i.Booking_ID
                             WHERE b.Booking_Status = 'Pending Approval'
                             ORDER BY b.Booking_ID DESC");
        while ($res && $r = $res->fetch_assoc()) {
            $pending[] = [
                'id' => $r['Booking_ID'],
...
elseif ($action == 'book_room') {
    ...
    // Booking
    $stmt = $conn->prepare("INSERT INTO booking (Customer_ID, Room_ID, Check_In_Date, Check_Out_Date, Booking_Status) VALUES (?, ?, ?, ?, 'Pending Approval')");
    $stmt->bind_param("iiss", $customerId, $roomId, $date, $checkout);
    $stmt->execute();
    $bookingId = $conn->insert_id;
    
    // Update room status
    $conn->query("UPDATE room SET Availability_Status = 'Pending' WHERE Room_ID = " . intval($roomId));
...
elseif ($action == 'add_customer_admin') {
    $data = json_decode(file_get_contents('php://input'), true);
    $name = $data['name'] ?? '';
    $email = $data['email'] ?? '';
    $phone = $data['phone'] ?? '';
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(["status" => "error", "message" => "Could not add customer"]);
    }
}
elseif ($action == 'approve_booking') {
    ...
    $res = $conn->query("SELECT Room_ID FROM booking WHERE Booking_ID = " . intval($bookingId));
    if ($row = $res->fetch_assoc()) {
        $roomId = $row['Room_ID'];
        // Update booking status
        $conn->query("UPDATE booking SET Booking_Status = 'Confirmed' WHERE Booking_ID = " . intval($bookingId));
        // Update room status
...
elseif ($action == 'reject_booking') {
    ...
    if ($row = $res->fetch_assoc()) {
        $roomId = $row['Room_ID'];
        // Update booking status
        $conn->query("UPDATE booking SET Booking_Status = 'Rejected' WHERE Booking_ID = " . intval($bookingId));
        // Update room status
        $conn->query("UPDATE room SET Availability_Status = 'Available' WHERE Room_ID = " . intval($roomId));
        echo json_encode(["status" => "success"]);
    } else {
